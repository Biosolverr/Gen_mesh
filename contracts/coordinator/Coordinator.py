...
"""
        ...
        #    expected agents (and, per agent, which capability they were
        #    assigned) before any agent can possibly report back.
        ...
        for agent_address, cap in assigned:
            aggregator.emit(on="finalized").add_expected_agent(
                task_id, agent_address.as_hex, cap
            )

        # 5. Execution Plan materializes as fan-out emit() — and nothing
        #    after. Coordinator does not wait for or verify results.
        #    Each agent now only accepts this call from this Coordinator's
        #    own address (see Agent.execute()'s coordinator-only check) —
        #    task content reaches an agent exclusively through this trusted
        #    dispatch, never from an arbitrary external caller.
        for agent_address, cap in assigned:
